...
const CityDashboard = () => {
  ...
  const handleExportRows = (rows: MRT_Row<City>[]) => {
    csvExporter.generateCsv(rows.map((row) => row.original));
  };

  const handleExportData = () => {
    csvExporter.generateCsv(sortedCities);
  };
  ...
  return (
    <Container style={{ marginTop: "7em" }}>
      ...
      <MaterialReactTable
        ...
        columns={columns}
        data={sortedCities} // Use sortedCities instead of cities
        enableRowSelection
        ...
        renderRowActions={({ row, table }) => (
          <Box sx={{ display: "flex", gap: "1rem" }}>
            ...
            >
              <IconButton
                onClick={() => deleteCity(parseInt(row.getValue("id")))}
              >
                <Delete />
              </IconButton>
            </Tooltip>
